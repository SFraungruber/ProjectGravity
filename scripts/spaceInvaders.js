...
var Player = SheetSprite.extend({
  ...
  handleInput: function() {
      if(newShoot && (this.bulletDelayAccumulator > 0.5)){
          this.bulletDelayAccumulator = 0;
          newShoot = false;
          this.shoot();
      }
  ...
  update: function(dt) {
    ...
    // apply x vel
    this.position.x = Number(updatePosition * (CANVAS_WIDTH/180));
  ...
  update: function(dt) {
    ...
    if (this.stepAccumulator >= this.stepDelay) {
      ...
      if (this.position.y > CANVAS_WIDTH - 50) {
        ...
        document.dispatchEvent(new CustomEvent("onGameOver", { "detail": {player:0, points: player.score, ranking:1} }));
...
function initCanvas() {
  ...
  window.addEventListener('resize', resize);
    document.addEventListener("onPositionUpdate", onPositionUpdate);
    document.addEventListener("onDisplayButtonPressed", onDisplayButtonPressed);
    document.addEventListener("onStartGame", onStartGame);
...
function animate() {
  ...
  var dt = now - lastTime;
  if (dt > 100) dt = 100;
  if (!hasGameStarted && OnGameStarted) {
      hasGameStarted = true;
    initGame();
  }
...
// ###################################################################
function resize() {
  var w = $('#game-canvas')[0].width;
  var h = $('#game-canvas')[0].height;
...
var onPositionUpdate = function(e){
    updatePosition = e.detail.beta;
};

var onDisplayButtonPressed = function(e){
    if(e.detail.button === 'A' || e.detail.button === 'B'){
        newShoot = true;
    }
};
